...
if ( $category > 0 ) {
  ...
  //   array(
  //     'key'     => 'rank_math_primary_ponencias_category',
  //     'value'   => $category,
  //     'compare' => 'LIKE'
  //   )
  // );
...
if ( ! $is_from_request && $attributes['hasFilters'] ) {
  ...
  ?>
  <div class="mtdev-presentations-filters">
    <form class="mtdev-presentations-filters__form" action="<?php echo $action ?>" method="GET">
      <!-- Category item -->
      <?php
        if ( $show_category_filter ) {
          ?>
          <div class="mtdev-presentations-filters__form-item">
            <label class="mtdev-presentations-filters__form-label" for="s-category">Categoría</label>
            <select class="mtdev-presentations-filters__form-input" name="s-category" id="s-category">
              <option value="-1">Todas</option>
              <?php
                $categories = get_terms( 'ponencias_category' );
      ...
      <!-- Sorting item -->
      <div class="mtdev-presentations-filters__form-item">
        <label class="mtdev-presentations-filters__form-label" for="s-order">Ordenar ponencias por</label>
        <select class="mtdev-presentations-filters__form-input" name="s-order" id="s-order">
      ...
      <!-- Search item -->
      <div class="mtdev-presentations-filters__form-item wp-block-search__button-inside wp-block-search__icon-button wp-block-search">
        <label class="mtdev-presentations-filters__form-label wp-block-search__label" for="s-search">Buscador</label>
        <div class="wp-block-search__inside-wrapper">
          <input class="mtdev-presentations-filters__form-input wp-block-search__input" type="search" name="s-search" id="s-search" value="<?php echo esc_attr( $search ); ?>" placeholder="Buscar ponencias">
...
if ( ! $is_from_request ) {
  echo '<section class="mtdev-presentations" data-attributes="' . esc_html( json_encode( $frontend_attrributes ) ) . '">';
}
if ( $latest_posts->have_posts() ) {
  while ( $latest_posts->have_posts() ) {
    $latest_posts->the_post();
    ?>
    <article class="mtdev-presentations-item">
      <a class="mtdev-presentations-item__link" href="<?php the_permalink(); ?>">Ver ponencia</a>

      <div class="mtdev-presentations-item__header">
        <div class="mtdev-presentations-item__thumbnail">
          <?php
            the_post_thumbnail();
          ?>
        </div>
      ...
      <div class="mtdev-presentations-item__info">
        <?php
        $id = get_the_id();
        ...
        if ( $post_category ) {
          ?>
          <ul class="mtdev-presentations-item__categories">
            <li class="mtdev-presentations-item__category">
              <a class="mtdev-presentations-item__category-link" href="<?php
        ...
        <h3 class="mtdev-presentations-item__title">
          <?php the_title(); ?>
        </h3>

        <a class="mtdev-presentations-item__button" href="<?php the_permalink(); ?>">
          Ver ponencia
        </a>
      </div>
    </article>
  ...
  ?>
  <div class="mtdev-presentations__no-results">
    <p class="mtdev-presentations__no-results-text">No se encontraron resultados <?php
      if ( $search ) {
