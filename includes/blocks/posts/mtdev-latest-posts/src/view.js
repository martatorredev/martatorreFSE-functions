...
const start = () => {
  ...
  const datasetAttributes = JSON.parse(postGrid.dataset.attributes)
  ...
  subscribeAtrributes(attributes => {
    ...
    if (!form && !postGrid.querySelector('.navigation')) return // Because the form and the pagination are not present
