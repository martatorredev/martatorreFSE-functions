...
const start = () => {
  ...
  const datasetAttributes = JSON.parse(posts.dataset.attributes)
  ...
  const form = document.querySelector('.mtdev-projects-filters__form')
  ...
  subscribeAtrributes(attributes => {
    ...
    if (!form && !posts.querySelector('.navigation')) return // Because the form and the pagination are not present
  ...
  posts?.addEventListener('click', onClickPagination)
...
const getEndpoint = () => {
  ...
  const endpoint = removeDoubleSlashes(`${wpJsonUrl}/mtdev/get_projects`)
